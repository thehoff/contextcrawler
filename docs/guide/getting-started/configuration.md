...
contextcrawler config --create   # create config file with defaults
```
...
[filters]
# These apply to file-reading commands (ls, find, grep, cat/contextcrawler read).
...
|----------|-------------|
| `CTXCRL_DISABLED=1` | Disable contextcrawler for a single command (`CTXCRL_DISABLED=1 git status`) |
| `CTXCRL_TEE_DIR` | Override the tee directory |
| `CTXCRL_TELEMETRY_DISABLED=1` | Disable telemetry |
| `CTXCRL_HOOK_AUDIT=1` | Enable hook audit logging |
| `SKIP_ENV_VALIDATION=1` | Skip env validation (useful with Next.js) |

## Tee system

When a command fails, contextcrawler saves the full raw output to a local file and prints the path:
...
```bash
CTXCRL_DISABLED=1 git rebase main
```

## Telemetry

contextcrawler sends one anonymous ping per day (23h interval). No personal data, no file paths, no command content.
