---
title: What contextcrawler Optimizes
description: Commands and ecosystems automatically optimized by contextcrawler with typical token savings
sidebar:
  order: 1
---
...
Once contextcrawler is installed with a hook, these commands are automatically intercepted and filtered. You run them normally — the hook rewrites them transparently before execution.
...
| `cat` / `head` / `tail <file>` | 60-80% | Smart file reading via `contextcrawler read` |
| `contextcrawler smart <file>` | 85% | 2-line heuristic code summary (signatures only) |
...
These flags apply to all contextcrawler commands and can push savings even higher:
...
contextcrawler git log --ultra-compact

# Debug: see what contextcrawler is doing
contextcrawler git status -vvv
```
...
If a command isn't in the list above, contextcrawler runs it through passthrough — the output reaches the LLM unchanged. You can explicitly track unsupported commands:

```bash
contextcrawler proxy make install    # runs make install, tracks usage, no filtering
```

To check which commands were missed opportunities: `contextcrawler discover`.
