# contextcrawler Technical Documentation

> **Start here** for a guided tour of how contextcrawler works end-to-end.
>
...
contextcrawler sits between the agent and the CLI, filtering outputs to keep only what matters. This achieves 60-90% token savings per command, reducing costs and increasing effective context window utilization. contextcrawler is a single Rust binary with no runtime dependencies beyond the compiled binary itself, adding less than 10ms overhead per command.
...
|  Intercepts: "git status" -> "contextcrawler git status"    |
+-------------------------+------------------------+
                          |
                          v
+--------------------------------------------------+
...
This is the full lifecycle of a command through contextcrawler, from LLM agent to filtered output.

### 3.1 Hook Installation (`contextcrawler init`)

The user runs `contextcrawler init` to set up hooks for their LLM agent. This:
...
4. Writes contextcrawler awareness instructions (e.g., `contextcrawler.md`) for prompt-level guidance

contextcrawler supports 7 agents, each with its own installation mode. The hook scripts are embedded in the binary and written at install time.
...
3. The hook calls `contextcrawler rewrite "git status"` as a subprocess
4. `contextcrawler rewrite` consults the command registry and returns `contextcrawler git status`
5. The hook sends a response telling the agent to use the rewritten command
6. If anything fails (jq missing, contextcrawler not found, no match), the hook exits silently -- the raw command runs unchanged
...
> **Details**: [`hooks/README.md`](../hooks/README.md) covers each agent's JSON format, the rewrite registry, compound command handling, and the `CTXCRL_DISABLED` override.

#### Rewrite Pipeline

The rewrite pipeline is how contextcrawler intercepts and rewrites commands. The call chain is:
...
LLM Agent: "cargo fmt --all && cargo test 2>&1 | tail -20"
  ...
  |  Reads JSON from agent, extracts command, calls `contextcrawler rewrite "$CMD"`
  |  On failure (jq missing, contextcrawler missing, old version): exit 0 (passthrough)
  |
  v
rewrite_cmd::run(cmd)                              [src/hooks/rewrite_cmd.rs]
  ...
  |     → None → exit(1)          (no contextcrawler equivalent, passthrough)
...
rewrite_command(cmd, excluded)                     [src/discover/registry.rs]
  ...
  |  - Simple "contextcrawler ..." (no operators) → return as-is
...
rewrite_segment(seg, excluded)                     [src/discover/registry.rs]
  ...
  |
  |  Step 4 — Already contextcrawler → return as-is
  |
  |  Step 5 — Special cases (short-circuit before classification)
  |  head -N / --lines=N → rewrite_line_range() → "contextcrawler read file --max-lines N"
  |  tail -N / -n N / --lines N → rewrite_line_range() → "contextcrawler read file --tail-lines N"
...
classify_command(cmd)                              [src/discover/registry.rs]
  ...
  |  2. Check IGNORED_PREFIXES (contextcrawler, mkdir, mv, ...)
...
LLM Agent executes rewritten command
  (bash handles && and |, each contextcrawler invocation is a separate process)
```
...
contextcrawler has two filter systems:
...
1. Guard: check if the command is an contextcrawler meta-command (`gain`, `init`, etc.) -- if so, show Clap error
...
Every command execution records metrics to SQLite (`~/.local/share/ctxcrl/tracking.db`):
...
Analytics commands (`contextcrawler gain`, `contextcrawler cc-economics`, `contextcrawler session`) query this database to produce dashboards and ROI reports.
...
1. Raw unfiltered output is saved to `~/.local/share/ctxcrl/tee/{epoch}_{slug}.log`
...
contextcrawler supports the following LLM agents through hook integrations:

| Agent | Hook Type | Mechanism | Can Modify Command? |
|-------|-----------|-----------|---------------------|
| Claude Code | Shell hook | `PreToolUse` in `settings.json` | Yes (`updatedInput`) |
| GitHub Copilot (VS Code) | Rust binary | `contextcrawler hook copilot` reads JSON | Yes (`updatedInput`) |
| GitHub Copilot CLI | Rust binary | `contextcrawler hook copilot` reads JSON | No (deny + suggestion) |
| Cursor | Shell hook | `preToolUse` hook | Yes (`updated_input`) |
| Gemini CLI | Rust binary | `contextcrawler hook gemini` reads JSON | Yes (`hookSpecificOutput`) |
...
Declarative filters with an 8-stage pipeline: strip ANSI, regex replace, match output, strip/keep lines, truncate lines, head/tail, max lines, on-empty message. Loaded from three tiers: built-in (compiled), global (`~/.config/ctxcrl/filters/`), project-local (`.ctxcrl/filters/`, trust-gated).
...
|--------|--------|--------------|
| Startup time | < 10ms | `hyperfine 'contextcrawler git status' 'git status'` |
| Memory usage | < 5MB resident | `/usr/bin/time -v contextcrawler git status` |
| Binary size | < 5MB stripped | `ls -lh target/release/ctxcrl` |
