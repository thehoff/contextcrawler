...
fn run_generic(subcommand: &str, args: &[String], verbose: u8, full_sub: &str) -> Result<i32> {
    ...
    if !output.status.success() {
        timer.track(
            &format!("aws {}", full_sub),
            &format!("contextcrawler aws {}", full_sub),
            &stderr,
            &stderr,
        );
...
fn run_s3_ls(extra_args: &[String], verbose: u8) -> Result<i32> {
    ...
    if !output.status.success() {
        ...
        timer.track("aws s3 ls", "contextcrawler aws s3 ls", &raw, &stderr);
        return Ok(exit_code);
    }
    ...
    timer.track("aws s3 ls", "contextcrawler aws s3 ls", &raw, &result.text);
    Ok(0)
}
