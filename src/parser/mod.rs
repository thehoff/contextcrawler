...
//!
//! The three-tier system ensures CTXCRL never returns false data silently.
...
pub trait OutputParser: Sized {
    ...
    /// 3. Return truncated passthrough with `[CTXCRL:PASSTHROUGH]` marker
...
pub fn truncate_output(output: &str, max_chars: usize) -> String {
    ...
    format!(
        "{}\n\n[CTXCRL:PASSTHROUGH] Output truncated ({} chars → {} chars)",
        truncated,
        chars.len(),
        max_chars
    )
...
pub fn emit_degradation_warning(tool: &str, reason: &str) {
    eprintln!("[CTXCRL:DEGRADED] {} parser: {}", tool, reason);
}

/// Helper to emit passthrough warning
pub fn emit_passthrough_warning(tool: &str, reason: &str) {
    eprintln!("[CTXCRL:PASSTHROUGH] {} parser: {}", tool, reason);
}
...
mod tests {
    ...
    fn test_truncate_output() {
        ...
        assert!(truncated.contains("[CTXCRL:PASSTHROUGH]"));
    ...
    fn test_truncate_output_multibyte() {
        ...
        assert!(result.contains("[CTXCRL:PASSTHROUGH]"));
