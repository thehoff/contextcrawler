...
Optional. Boots a read-only HTTP surface on `127.0.0.1` that renders the
same `Tracker` queries the text-mode subcommands use, plus the two
security JSONL logs the gate writes. Auto-shutdown after 1 h idle.
No auth (loopback-only by design).

| Command | Purpose | Issue |
|---|---|---|
| `contextcrawler gain --web` | Boot the dashboard on a free port and open the browser. | [#162](https://github.com/thehoff/contextcrawler/issues/162) |
| `contextcrawler gain --web --port N` | Pin the port (handy for shells where the auto-open is noisy). | [#162](https://github.com/thehoff/contextcrawler/issues/162) |
| `contextcrawler gain --web --no-browser` | Print the URL only — for SSH / headless / containerised use. | [#162](https://github.com/thehoff/contextcrawler/issues/162) |
| Seven panes | Summary, By day, Weak filters, Parse failures, Release boundaries, Security, Installs, Insights (stub). | [#162](https://github.com/thehoff/contextcrawler/issues/162) / [#171](https://github.com/thehoff/contextcrawler/issues/171) / [#172](https://github.com/thehoff/contextcrawler/issues/172) |
| Per-project install ledger | Every detected install gets a row keyed on CWD at install time. Tells you what got installed where, with what verdict, when. | [#172](https://github.com/thehoff/contextcrawler/issues/172) |

See the [`## Dashboard`](#dashboard) section below for screenshots and
the real-world `pi.dev` block example.
...
`contextcrawler gain --web` boots a local read-only HTTP server on
`127.0.0.1`, then opens a browser to a single-page UI that surfaces
every tracked metric ContextCrawler already collects. Loopback-only,
no auth, auto-shutdown after 1 h of idle so the process never lingers.
...
The screenshots below are from a synthetic dataset generated by
`scripts/seed-demo-env.sh` (so no personal paths leak); your own
dashboard will look identical against your real history.

### Summary

The headline view. Lifetime totals, average savings, and the top tools
by tokens saved. Pure read from `history.db` — no extra capture step.

<p align="center">
  <img src="docs/assets/dashboard-1-summary.png" alt="Dashboard — Summary pane: lifetime totals, top tools by savings" width="900">
</p>

### By day

Daily savings as a hand-rolled SVG sparkline (no Chart.js — the whole
frontend is one inlined HTML file), plus a 30-day detail table.

<p align="center">
  <img src="docs/assets/dashboard-2-byday.png" alt="Dashboard — By day pane: pink-to-purple sparkline and detail table" width="900">
</p>

### Weak filters

Tools ranked by leaked tokens (input that reached the model despite the
filter). Sliced from the latest release boundary so newly-released
filter behaviour isn't masked by months of pre-upgrade leakage.

<p align="center">
  <img src="docs/assets/dashboard-3-weak.png" alt="Dashboard — Weak filters: leaderboard ranked by leaked tokens" width="900">
</p>

### Parse failures

Commands that fell back to raw passthrough because the filter couldn't
parse the output. Target recovery rate is 95% — anything lower is an
RTK-level bug, not a user problem.

<p align="center">
  <img src="docs/assets/dashboard-4-failures.png" alt="Dashboard — Parse failures: recovery rate and recent fallbacks" width="900">
</p>

### Release boundaries

Every recorded `contextcrawler --version` upgrade observed. Useful for
before / after comparisons — "did `gh pr view` savings actually improve
after v0.1.10?".

<p align="center">
  <img src="docs/assets/dashboard-5-boundaries.png" alt="Dashboard — Release boundaries: install history with the latest pill" width="900">
</p>

### Security

Tirith gate downgrades + supply-chain verdicts in one place. Block
rate, top fired rules, most-blocked packages, recent block timeline.
Read from `~/Library/Application Support/contextcrawler/{downgrades,supply_chain}.jsonl`
with a 1 MiB tail cap.

<p align="center">
  <img src="docs/assets/dashboard-6-security.png" alt="Dashboard — Security: gate downgrades and supply-chain verdicts" width="900">
</p>

### Installs (per-project ledger)

Every install command the supply-chain gate sees lands as a row keyed
on the CWD at install time. Project dropdown filters the ledger so
"what got pip-installed into the api-server in the last 30 days" is one
click. Live events capture CWD; historical events backfilled from
`supply_chain.jsonl` carry empty CWD.

<p align="center">
  <img src="docs/assets/dashboard-7-installs.png" alt="Dashboard — Installs: per-project ledger with verdict pills" width="900">
</p>

### Real example: catching a fresh release

The gate isn't theoretical. On 2026-05-23 I went to install the
[Pi.Dev coding agent](https://pi.dev/) globally:

```bash
npm install -g --ignore-scripts @earendil-works/pi-coding-agent
```

ContextCrawler blocked it:

```text
[contextcrawler supply-chain] BLOCKED
  @earendil-works/pi-coding-agent [npm]:
    RecentRelease — version 0.75.4 published 2.4 days ago,
    under the 3-day cooldown.
    Severity: HIGH

  Overrides: rerun with CONTEXTCRAWLER_SUPPLY_CHAIN=off,
  or add the package to ~/.config/contextcrawler/supply-chain.toml
  [overrides.always_allow]
```

The cooldown is a defence against the most common npm supply-chain
pattern: a fresh version published in the last few hours, then yanked
once the malicious payload spreads. ContextCrawler holds the install
until the version has been live long enough for the ecosystem to
react. Override is one env var — not zero — so the friction is the
point.

The same event lands in the Security and Installs panes, so the block
is reviewable after the fact, not just at install time.

### Endpoints

The SPA fetches JSON. They're stable, documented, and safe to hit from
your own tools (jq, curl, a dashboard of your own):

| Endpoint | Returns |
|---|---|
| `GET /api/summary` | Lifetime totals + top tools + last 30 days `(date, saved)` pairs. |
| `GET /api/by-day` | `DayStats[]` for the full recorded history. |
| `GET /api/weak-filters` | Tools ranked by leaked tokens, sliced from the latest release boundary. |
| `GET /api/failures` | Parse-failure summary: total, recovery rate, top-failing commands, recent failures. |
| `GET /api/boundaries` | Every recorded `release_boundaries` row + the latest pointer. |
| `GET /api/security/gate` | Tirith downgrades log: totals, by-action, by-rule (top 10), recent (20). |
| `GET /api/security/supply-chain` | Supply-chain check log: totals, by-verdict, most-blocked packages, recent blocks. |
| `GET /api/installs?project=<path>&limit=<n>` | Per-project install ledger. `project=` empty/omit for all projects. |
| `GET /api/insights` | Stub for [#158](https://github.com/thehoff/contextcrawler/issues/158); returns `{status:"pending", insights:[]}` until that lands. |
...
<details>
<summary><strong>3. Dashboard data plane — what feeds the `gain --web` panes</strong></summary>

```mermaid
flowchart TB
    AGENT["agent command"]
    HOOK["contextcrawler hook"]
    TRACKER["history.db<br/>SQLite tracker"]
    SC_JSONL["supply_chain.jsonl<br/>audit log"]
    GATE_JSONL["downgrades.jsonl<br/>Tirith audit log"]
    INSTALLS_TBL["installs table<br/>per-project ledger"]

    SERVER["gain --web<br/>tiny_http on 127.0.0.1"]
    BROWSER["browser<br/>single-file SPA"]

    AGENT --> HOOK
    HOOK -- "metrics" --> TRACKER
    HOOK -- "gate event" --> GATE_JSONL
    HOOK -- "install event" --> SC_JSONL
    HOOK -- "install mirror" --> INSTALLS_TBL
    SC_JSONL -. "first-boot backfill" .-> INSTALLS_TBL
    ...
    classDef hot fill:#2a0a2e,stroke:#e83e8c,color:#fff
    classDef cold fill:#1a1a2e,stroke:#888,color:#ddd
    class TRACKER,INSTALLS_TBL,SERVER hot
    class SC_JSONL,GATE_JSONL cold
```

| Node | Source of truth for | Caption |
|---|---|---|
| `history.db` | command metrics, parse failures, release boundaries | written on every hook invocation; queried by all `gain` commands |
| `installs` table | per-project install ledger | mirrored from the gate hook; backfilled once from the JSONL |
| `supply_chain.jsonl` | immutable supply-chain audit log | append-only; never the query surface |
| `downgrades.jsonl` | immutable Tirith downgrade audit log | append-only; tail-read for the Security pane |
| `gain --web` | the dashboard surface | read-only, loopback-only, auto-shutdown after 1 h idle |
